...
export class PgTransactionRepository implements TransactionRepository {
  async create(dto: CreateTransactionDto): Promise<TransactionRecord> {
    const result = await query<TransactionRow>(
      `
        ...
        values ($1, $2, $3, $4)
        returning id::text as id, user_id, amount, type, description, created_at;
      `,
      [dto.userId, dto.amount, dto.type, dto.description ?? null]
    );
  ...
  async findById(id: string): Promise<TransactionRecord | null> {
    const result = await query<TransactionRow>(
      `
        select id::text as id, user_id, amount, type, description, created_at
        from transactions
        where id = $1
        limit 1;
      `,
